...
static void
run_buffered_binding(struct controller_ctx *ctx,
...
static void send_garp_prepare(struct controller_ctx *ctx,
...
static void prepare_ipv6_ras(const struct controller_ctx *,
                             struct hmap *local_datapaths)
    OVS_REQUIRES(pinctrl_mutex);
 ...
 * thread context. */
static void
sync_dns_cache(const struct controller_ctx *ctx)
    OVS_REQUIRES(pinctrl_mutex)
{
            ...
            struct sset *active_tunnels)
{
    ...
    send_garp_prepare(ctx, br_int, chassis, chassis_index,
                      local_datapaths, active_tunnels);
    prepare_ipv6_ras(ctx, local_datapaths);
    sync_dns_cache(ctx);
    run_buffered_binding(ctx, local_datapaths);
    ovs_mutex_unlock(&pinctrl_mutex);
}
 ...
 * thread context. */
static void
prepare_ipv6_ras(const struct controller_ctx *ctx,
                 struct hmap *local_datapaths)
    OVS_REQUIRES(pinctrl_mutex)
{
...
mac_binding_lookup(struct controller_ctx *ctx,
                   const struct sbrec_port_binding *pb,
                   const char *ip)
{
    const struct sbrec_mac_binding *b;
    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    HMAP_FOR_EACH (ld, hmap_node, local_datapaths) {
        ...
        target = sbrec_port_binding_index_init_row(ctx->ovnsb_idl,
                                                   &sbrec_table_port_binding);
        ...
        const struct sbrec_port_binding *pb;
        SBREC_PORT_BINDING_FOR_EACH_EQUAL (pb, &cursor, target) {
            struct buffered_packets *cur_qp, *next_qp;
            HMAP_FOR_EACH_SAFE (cur_qp, next_qp, hmap_node,
                ...
                const struct sbrec_mac_binding *b =
                    mac_binding_lookup(ctx, pb, ds_cstr(&ip_s));
 ...
 * thread context. */
static void
send_garp_prepare(struct controller_ctx *ctx,
                  const struct ovsrec_bridge *br_int,
                  const struct sbrec_chassis *chassis,
                  const struct chassis_index *chassis_index,
                  struct hmap *local_datapaths,
