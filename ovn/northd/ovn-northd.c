...
extract_dhcp_options_from_json(struct json *json, struct smap *dhcp_options)
{
    ...
    /* Calling 'smap_from_json()' asserts if 'json' values are not string
     * and we don't want to assert here.
     * */
...
static struct dhcp_opts_map supported_dhcp_opts[] = {
    DHCP_OPTION("offerip", 0, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("netmask", 1, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("router", 3, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("dns_server", 6, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("log_server", 7, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("lpr_server", 9, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("swap_server", 16, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("policy_filter", 21, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("router_solicitation", 32, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("nis_server", 41, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("ntp_server", 42, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("server_id", 54, DHCP_OPT_TYPE_IP4),
    DHCP_OPTION("tftp_server", 66, DHCP_OPT_TYPE_IP4),
    ...
    DHCP_OPTION("tcp_ttl", 37, DHCP_OPT_TYPE_UINT8),
    DHCP_OPTION("mtu", 26, DHCP_OPT_TYPE_UINT16),
    DHCP_OPTION("lease_time", 51, DHCP_OPT_TYPE_UINT32),
};

static void
check_add_supported_dhcp_opts_to_sb_db(struct northd_context *ctx)
{
    ...
    struct dhcp_opts_map *opt, *next_opt;
    HMAP_FOR_EACH_SAFE(opt, next_opt, hmap_node, &dhcp_opts_to_add) {
        ...
        hmap_remove(&dhcp_opts_to_add, &opt->hmap_node);
    }
...
main(int argc, char *argv[])
{
    ...
    while (!exiting) {
        ...
        ovnsb_db_run(&ctx);
        if (ctx.ovnsb_txn) {
             check_add_supported_dhcp_opts_to_sb_db(&ctx);
        }
