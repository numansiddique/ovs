...
sync_ha_chassis_group_for_sbpb(struct northd_context *ctx,
    ...
    sbrec_port_binding_set_ha_chassis_group(pb, sb_ha_chassis_group);
}

/* This functions translates the gw chassis on the nb database
 * to ha_chassis_group sb database entries. */
static void
    ...
    if (op->nbrp) {
        ...
        if (op->derived) {
            ...
            if (op->nbrp->ha_chassis_group) {
                ...
                sync_ha_chassis_group_for_sbpb(ctx, op->nbrp, op->sb);
        ...
        if (new_sb_chassis_group ||
            ...
            for (size_t i = 0; i < nb_ha_chassis_group->n_ha_chassis; i++) {
                ...
                if (!chassis) {
                    ...
                    VLOG_WARN_RL(&rl, "chassis name '%s' not found for the "
                                 "ha_chassis row. Ignoring the HA chassis "
                                 "group : %s", nb_ha_chassis->chassis_name,
                                 nb_ha_chassis_group->name);
                    break;
                }
                                   ...
                                   struct shash *ha_ref_chassis_map)
{
    for (size_t i = 0; i < op->od->n_router_ports; i++) {
        ...
        for (size_t j = 0; j < ref_ch_info->n_ref_chassis; j++) {
            if (sb->chassis == ref_ch_info->ref_chassis[j]) {
                present = true;
                break;
            }
        }
        ...
        if (!present) {
            ref_ch_info->ref_chassis = xrealloc(
                            ...
                            struct shash *ha_ref_chassis_map)
{
    ...
    SBREC_PORT_BINDING_FOR_EACH(sb, ctx->ovnsb_idl) {
        ...
        if (ctx->ovnsb_txn && !strcmp(sb->type, "") &&
            op->od->n_router_ports && sb->chassis) {
