    ...
    if (op->nbrp) {
        ...
        if (strcmp(op->nbsp->type, "router")) {
            ...
            if (!strcmp(op->nbsp->type, "external")) {
                /* Get the sbrec_ha_chassis_group and set it. */
                const struct sbrec_ha_chassis_group *sb_ha_ch_grp = NULL;
                if (op->nbsp->ha_chassis_group) {
                    sb_ha_ch_grp = ha_chassis_group_lookup_by_name(
                        ctx->sbrec_ha_chassis_grp_by_name,
                        op->nbsp->ha_chassis_group->name);
                }
                sbrec_port_binding_set_ha_chassis_group(op->sb, sb_ha_ch_grp);
            }
        } else {
