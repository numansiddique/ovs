...
build_acls(struct ovn_datapath *od, struct hmap *lflows)
{
    ...
    for (size_t i = 0; i < od->nbs->n_ports; i++) {
        ...
        if (!dns_flow_added && od->nbs->ports[i]->dns) {
            const char *actions = has_stateful ? "ct_commit; next;" :
                "next;";
            ovn_lflow_add(
...
sync_dns_entries(struct northd_context *ctx, struct hmap *datapaths)
{
    ...
    SBREC_DNS_FOR_EACH_SAFE(sbrec_dns, next, ctx->ovnsb_idl) {
        ...
        HMAP_FOR_EACH_WITH_HASH(dns_info, hmap_node, hash, &dns_map) {
            if (!strcmp(dns_info->dns->hostname, sbrec_dns->hostname)) {
                ...
                if ((dns_info->dns->n_ipv4_addresses !=
                     sbrec_dns->n_ipv4_addresses) ||
                    (dns_info->dns->n_ipv6_addresses !=
                ...
                for (size_t i = 0; i < sbrec_dns->n_ipv4_addresses; i++) {
                    if (strcmp(dns_info->dns->ipv4_addresses[i],
                ...
                for (size_t i = 0; i < sbrec_dns->n_ipv6_addresses; i++) {
                    if (strcmp(dns_info->dns->ipv6_addresses[i],
    ...
    HMAP_FOR_EACH_POP(dns_info, hmap_node, &dns_map) {
        ...
        sbrec_dns_set_fqdn(sbrec_dns, dns_info->dns->fqdn);
        sbrec_dns_set_ipv4_addresses(
            sbrec_dns, (const char **) dns_info->dns->ipv4_addresses,
            dns_info->dns->n_ipv4_addresses);
        sbrec_dns_set_ipv6_addresses(
            sbrec_dns, (const char **) dns_info->dns->ipv6_addresses,
            dns_info->dns->n_ipv6_addresses);
