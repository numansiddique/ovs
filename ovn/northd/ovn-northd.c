              ...
              struct hmap *ports, struct hmap *monitor_map)
{
    ...
    SMAP_FOR_EACH (node, &nbrec_lb->vips) {
        ...
        for (token = strtok_r(tokstr, ",", &save_ptr);
            ...
            const char *lport = smap_get(&nbrec_lb->ip_port_mappings, backend_ip);
            struct ovn_port *op = NULL;
            if (lport) {
                op = ovn_port_find(ports, lport);
            }
        ...
        for (size_t j = 0; j < lb->n_vips; j++) {
            ...
            if (lb_vip->health_check) {
                ...
                for (size_t k = 0; k < lb_vip->n_backends; k++) {
                    ...
                    if (is_up) {
                        n_active_backends++;
                        ds_put_format(&action, "%s:%"PRIu16",", backend->ip, backend->port);
                    }
                }
             ...
             struct ovs_list *lr_list)
{
    ...
    const char *monitor_svc_mac_str = smap_get(&nb->options, "monitor_svc_mac");
