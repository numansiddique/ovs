                    ...
                    struct ds *resume_action OVS_UNUSED)
{
    ...
    ds_put_format(resume_action, "eth.dst = eth.src; eth.src = %s; "
                  "ip6.dst = %s; ip6.src = %s; udp.src = 547; "
                  "udp.dst = 546; outport = inport; inport = \"\";"
                  " /* Allow sending out inport. */ output;",
                  server_mac, ia_addr, server_ip);
#else
    ds_put_format(pause_action, "eth.dst = eth.src; eth.src = %s; "
                  "ip6.dst = ip6.src; ip6.src = %s; udp.src = 547; "
                  "udp.dst = 546; ",
                  server_mac, server_ip);
    ds_put_format(pause_action, "put_dhcpv6_opts(reg0, SERVER_ID = %s, "
                  "IA_ADDR = %s, ", server_mac, ia_addr);
